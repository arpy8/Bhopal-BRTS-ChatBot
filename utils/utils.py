...
def find_closest_bus_station(df, current_lat, current_lon):
    ...
    closest_point["dir_to_closest_point"] = f"https://www.google.com/maps/dir/{current_lat},{current_lon}/{closest_point['latitude']},{closest_point['longitude']}"
    
    closest_point["all_routes"] = list(df[df['station'] == closest_point['station']]['route'].unique())
...
def first_message(df, user_lat, user_long):
    ...
    for word in MESSAGE_TEMPLATE[0].format(station=closest_bus_station['station'], 
                                            distance=round(closest_bus_station['distance'], 2), 
                                                routes=" ".join(closest_bus_station['all_routes'])).split(" "):
        yield word + " "
        time.sleep(random.uniform(0.06, 0.1))
        
    st.link_button("Navigate using Google Map", url=closest_bus_station["dir_to_closest_point"], use_container_width=True)
